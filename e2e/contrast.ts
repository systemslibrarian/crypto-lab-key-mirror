 ...
 *
 *  - text over a TRANSLUCENT surface. This page draws no gradients, but every
 *    state fill it paints is a `color-mix(..., transparent)` wash — `.chip-ok`,
 *    `.chip-alarm`, `.chip-warn`, `.hex-diff`, `.flow-alarm`, `.flow-done`,
 *    `.evil-toggle`, `.cl-hero-why`. A wash that mixes toward `transparent` is
 *    a translucent fill, so its text composites onto whatever is behind it —
 *    the card, then the panel, then the page — rather than onto a flat colour
 *    axe can read off one element.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab is careful about it
 *    (`.flow-idle` and `.steps li.step-pending` both set `opacity: 1` and dim
 *    with `color` instead, which is the right call), but `.cl-hero-sub` still
 *    renders at `opacity: .85`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.label-scroll`
 *    caps 22 VRF label rows at `max-height: 17rem` with `overflow: auto`, and
 *    `.tracewrap` and `.flow-track` scroll horizontally. Content scrolled past
 ...
 *    through to white, inventing failures for rows nobody can see.
 *
 ...
 *    draws no SVG text — its only SVG is the shared header's icon set and the
 *    `--chevron` data URI, neither of which carries a `<text>` — but the rule is
 *    kept because it is cheap and silent when unused: an `<svg>` draws a shape
 *    as a `<rect>` and then its
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: this page mixes its state
     * hues into surfaces with `color-mix()` in both `srgb` (every chip, every
     * alarm wash) and `oklab` (the hero's "why it matters" box, the flow node
     * focus ring), and Chromium reports those to `getComputedStyle` unchanged
     ...
     *
     * The only `background-image` on this page is the `--chevron` data URI on
     * the layer radios, which is a `url()` and paints nothing this walk can
     * read — so on today's stylesheet the gradient path always takes the
     * `TRANSPARENT` early return. It is kept because it is silent when unused
     * and because the form it replaced was actively wrong: an older version
     * judged every gradient at its WORST STOP, assuming that stop covered the
     * text wherever the text sat. That holds for a gradient spanning its element
     * and fails for a small positioned one — in one lab it invented a 3.56:1
     * reading for footer links that render ~4.9:1. So each gradient is *sampled*
     * at the text's real location instead: linear by projecting onto the
     * gradient line, radial by distance from the centre over the farthest-corner
     * radius. A non-gradient layer (`url()`, `none`) paints nothing measurable.
     */
     ...
     *
     * This page renders a `<span>` per hex NIBBLE PAIR across several diff
     * views, plus 22 VRF label rows and a full consistency-proof trace table —
     * many hundreds of text-owning elements — and every one of them walks the
     ...
     * surface it happens to sit on and INVENTS failures. This lab's `.sr-only`
     * is exactly the classic recipe (`clip: rect(0 0 0 0)`). Deliberately
     * narrow — only a ZERO-area clip qualifies, so a real partial clip is still
     * measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosures are the `details.expert`
      // "expert view" notes under several exhibits, whose open state is a real
      // state the gate reaches by clicking their own summaries.
     ...
     * that covers every `.chip-icon` (✓/✗/⚠/·) and every `.flow-icon`, each of
     * which is an emoji or glyph sitting beside words that carry the same
     * meaning — and an emoji paints in its own multicolour, so measuring it
     * against the element's declared `color` describes nothing on screen.
     */
